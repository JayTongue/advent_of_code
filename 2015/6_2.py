...
bool_table = [[0 for _ in range(1000)] for _ in range(1000)]
for command, corner_1, corner_2 in all_moves:
    for x in range(min(corner_1[0], corner_2[0]), max(corner_1[0], corner_2[0])+1):
        for y in range(min(corner_2[1], corner_1[1]), max(corner_2[1], corner_1[1])+1):
                if command == 'toggle':
                    bool_table[x][y] = bool_table[x][y]+2
                elif command == 'on':
                    bool_table[x][y] = bool_table[x][y]+1
                else:
                    bool_table[x][y] = max(bool_table[x][y]-1, 0)
