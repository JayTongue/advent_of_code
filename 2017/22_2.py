...
for _ in range(int(1e7)):
    if grid[virus_pos[0]][virus_pos[1]] == '.':
        grid[virus_pos[0]][virus_pos[1]] = 'W'
        new_direction = turn(virus_ori, 'left')
        moved = move(virus_pos, new_direction)
    elif grid[virus_pos[0]][virus_pos[1]] == 'W':
        grid[virus_pos[0]][virus_pos[1]] = '#'
        new_direction = virus_ori
        moved = move(virus_pos, new_direction)
        activated += 1
    elif grid[virus_pos[0]][virus_pos[1]] == '#':
        grid[virus_pos[0]][virus_pos[1]] = 'F'
        new_direction = turn(virus_ori, 'right')
        moved = move(virus_pos, new_direction)
    elif grid[virus_pos[0]][virus_pos[1]] == 'F':
        grid[virus_pos[0]][virus_pos[1]] = '.'
        new_direction = turn(turn(virus_ori, 'left'), 'left')
        moved = move(virus_pos, new_direction)
