...
def rank(fives, fours, fulls, threes, twos, ones, high):
	rank_dict = {'A':14, 'K':13, 'Q':12, 'J':1, 'T':10, '9':9, '8':8, '7':7, '6':6, '5':5, '4':4, '3':3, '2':2}
	big_list = []
...
def sort_hands(data):
	...
	def screw_wilds(hand):
		if hand['J'] == 5:
			return hand
		elif 'J' not in hand:
			return hand
		else:
			max_key = max((k for k in hand if k != 'J'), key=lambda k: hand[k])
			hand[max_key] += hand['J']
			del hand['J']
			return hand
	...
	for cards, bid in data:
		hand = Counter(cards)
		j_hand = screw_wilds(hand)
		j_hand = sorted(j_hand.values(), reverse=True)
		if 5 in j_hand:
			fives.add((cards, bid))
		elif 4 in j_hand:
			fours.add((cards, bid))
		...
		elif j_hand.count(2) == 2:
			twos.add((cards, bid))
		elif j_hand.count(2) == 1:
			ones.add((cards, bid))		
		else:
			high.add((cards, bid))
