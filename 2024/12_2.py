    ...
    for point in chunk:
        ...
        for neighbor in neighbors:
            try:
                ...
                perimeter += 1
                peri_coords.append(neighbor)
    sides = calculate_sides(chunk)

    # return(sides)


def calculate_sides(chunk):
    # from top
    top_uniques = set()
    for x, y in chunk:
        top_uniques.add(x)
    print(chunk, top_uniques)

    top_uniques = list(top_uniques).sort()
    old_unique = -2
    for top_unique in top_uniques:
        if old_unique + 1 == top_unique:
            old_unique = top_unique
            continue
        else:
            sides_at_level = []
            for coord in chunk:
                x, y = coord
                if x == top_unique:
                    sides_at_level.append(y)
            
            sides_at_level.sort()
            print(sides_at_level)
