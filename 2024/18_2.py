...
def main():
    ...
    solution = 0
    while solution != -1:
        data_window = data[:break_point]

        data_map = populate_map(data_window, map_size)

        solution = solve_maze(data_map, (0, 0), map_size)
        if solution == -1:
             print(f'Break Point Found: {break_point-1}, at coord {data[break_point-1]}')
             break
        print(break_point, solution)
        break_point += 1
